...
// #include <ncurses.h>
/*
    Cоздание терминального интерфейса с использованием ncurses
    Используйте библиотеку ncurses для создания текстового интерфейса. Это
   включает:

    1. Инициализацию ncurses с помощью initscr()
    2. Отрисовку игрового поля и фигур на экране
    3. Обработку ввода пользователя для управления движением и поворотом фигур
    4. Отображение счета и сообщения об окончании игры
*/
...
void GetNewShape() {  // returns random shape
  ...
  // /\ выбирает начальную позицию новой фигуры по оси X на игровом поле,
  // учитывая ширину фигуры и общее количество столбцов на поле.
  new_shape.row = 0;
  // DeleteShape(current);
  current = new_shape;
...
int main() {
  ...
  int c;
  initscr();
  struct timeval before, after;  // структура определяется в заголовочном файле
  ...
  while (GameOn) {
    ...
    if (((double)after.tv_sec * 1000000 + (double)after.tv_usec) -
        ...
        timer) {  // time difference in microsec
      // accuracy
      before = after;
      // ManipulateCurrent('s');
    }
